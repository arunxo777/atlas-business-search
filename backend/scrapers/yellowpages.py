"""YellowPages async scraper using crawl4ai."""
...
from scrapers.listing_parser import extract_from_yellowpages
...
class YellowPagesScraper:
    ...
    async def scrape_search(
        ...
        for page_num in range(max_pages):
            ...
            try:
                ...
                parsed = extract_from_yellowpages(html, current_url, self.job_id)
                if parsed:
                ...
                for detail_url in detail_urls:
                    ...
                    records = await self.generic.scrape_url(detail_url, "yellowpages")
                ...
                logger.error("YellowPages page %d error: %s", page_num, exc)
                break

        if not all_records:
            records = await self.generic.scrape_url(search_url, "yellowpages")
            all_records.extend(records)
    ...
    def _parse_listing_urls(self, html: str) -> list[str]:
        ...
        urls: list[str] = []
        for link in soup.select("a.business-name, a[href*='/mip/'], a[href*='yellowpages.com/mip']"):
            href = link.get("href", "")
            if href:
                full = urljoin(YP_BASE, href)
                if "/mip/" in full and full not in urls:
                    urls.append(full)
        return urls[:30]

    def _parse_next_page(self, html: str) -> str | None:
        soup = BeautifulSoup(html, "lxml")
        next_link = soup.select_one("a.next, a[aria-label='Next']")
        if next_link and next_link.get("href"):
            return urljoin(YP_BASE, next_link["href"])
        return None

    async def scrape_url(self, url: str) -> list[BusinessRecord]:
        return await self.generic.scrape_url(url, "yellowpages")
